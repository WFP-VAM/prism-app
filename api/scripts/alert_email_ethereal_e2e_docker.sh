#!/usr/bin/env bash
# AA alert email E2E: flood AA + storm AA + threshold → Ethereal (Chromium in api image).
#
#   make -C api api-alert-email-e2e
#   # or: ./scripts/alert_email_ethereal_e2e_docker.sh
#
# Optional env on host: PRISM_E2E_* — see prism_app/tests/test_alert_email_e2e_ethereal.py (three tests).
...
docker compose run --rm --no-deps \
  ...
  -e PRISM_E2E_THRESHOLD_API_URL \
  -e PRISM_E2E_THRESHOLD_LAYER_ID \
  -e PRISM_E2E_THRESHOLD_LAYER_NAME \
  -e PRISM_E2E_THRESHOLD_LAYER_TITLE \
