...
def write_alerts(alerts_model: AlertsModel):
    ...
    try:
        data = jsonable_encoder(alerts_model)
...
def stats_demo(geojson_out: Optional[str] = False, group_by: Optional[str] = None,
    ...
    parameter_intersect_comparison_string = intersect_comparison

    intersect_comparison = None
    if parameter_intersect_comparison_string is not None:
        intersect_comparison = validate_intersect_parameter(
            parameter_intersect_comparison_string
        )
