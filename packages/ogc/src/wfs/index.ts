...
export class WFS extends Base {
  ...
  async getLayer(layerId: string, { fetch: _fetch }: { fetch?: any }): Promise<FeatureLayer> {
    this.checkLayer(layerId);
    return new FeatureLayer({ capabilities: this.capabilities, fetch: _fetch, id: layerId });
  }
}
