...
test("WCS for data cube", async ({ eq }) => {
  const wcs = new WCS("https://mongolia.sibelius-datacube.org:5000/wcs", { fetch });
  const layer = await wcs.getLayer("10DayTrend");
  const dates = await layer.getLayerDates();
  console.log("got layer dates");
  eq(dates.length, 29);
  eq(dates.every(d => typeof d === "string"), true);
  eq(dates[0], '2019-05-21T00:00:00.000Z');

  const extent = await layer.getExtent();
  eq(extent, [86.7469655846003,41.4606540712216,117.717378164332,52.3174921613588]);

  // use WCS extent as bbox
  const url1 = await layer.getImageUrl({ bbox_digits: 1, format: "GeoTIFF", height: 500, width: 500 });
  eq(url1, "https://mongolia.sibelius-datacube.org:5000/wcs?bbox=86.7%2C41.5%2C117.7%2C52.3&coverage=10DayTrend&crs=EPSG%3A4326&format=GeoTIFF&height=500&request=GetCoverage&service=WCS&version=1.0.0&width=500");

  // with bbox
  const url2 = await layer.getImageUrl({ bbox: [100, 45, 103.09704125797317, 46.08568380901372], format: "GeoTIFF", height: 222, width: 677 });
  eq(url2, "https://mongolia.sibelius-datacube.org:5000/wcs?bbox=100%2C45%2C103.09704125797317%2C46.08568380901372&coverage=10DayTrend&crs=EPSG%3A4326&format=GeoTIFF&height=222&request=GetCoverage&service=WCS&version=1.0.0&width=677");

  const image = await layer.getImage({ bbox: [100, 45, 103.09704125797317, 46.08568380901372], format: "GeoTIFF", height: 222, width: 677 });
